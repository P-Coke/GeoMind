...
GeoMind 是一个面向 Earth Engine 的 AI 驱动桌面客户端。它尝试把 GEE 的资产、脚本、任务和地图工作区整合到一个本地应用里，让用户既可以用模板化工具完成常见遥感任务，也可以逐步过渡到 AI 辅助脚本生成与执行。

**！！！当前仓库处于早期演示阶段，重点是验证桌面工作区、GEE 连接、ROI/图层交互、AI 草案生成和运行链路，而不是提供稳定的生产版本。**
...
- 已具备：桌面 UI、项目/运行页面、基础 GEE 认证、ROI 上传、模板工具、AI 草案生成、运行记录与截图展示能力。
